...
async function queueFiles(files) {
  for (const file of files) {
    ...
    const folder = state.tab === "photoshoots" ? PHOTOS_PATH : BRAND_PATH;
    const safeName = file.name.replace(/[^\w.\-]+/g, "_");
    const path = `${folder}/${safeName}`;
    state.queueUploads.push({ file, path, base64 });

    // also add to list immediately (preview)
    const list = activeList();
    list.push({ src: path, alt: "", caption: "", order: list.length });
  }
  setDirty();
  renderGrid();
}
